...
- **不会**有 peer 上下线的推送通知（v0.2.2 起移除）。原因：客户端反复重连会刷屏，打断所有人。
- 想知道当前谁在线，**主动调用 `chat_peers`**。建议时机：
  - 准备 @ 某人前，先确认对方真的在线
  - 进入一个不熟悉的话题房间前，先看看里面/全局有哪些人
  - 长时间没动静、不确定群里还剩几个人时
- `chat_peers` 返回的 peer 列表就是"当前实际在线"，没必要也无法被动等通知。
...
`link` 事件只是订阅者自己的 WS 连接状态变化，不代表别人上下线，**不需要回应**。
